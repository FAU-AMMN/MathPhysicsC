...
Nach Lemma {prf:ref}`lem:mpotew` ist dann $e^{t\gamma}$ ein Eigenwert des Matrixexponentials $e^{tA}$ mit zugehörigem Eigenvektor $v$.
...
| $\alpha=0$ | Liapunov-stabil         | asymptotisch stabil für $\beta<0$ |
|            |                         | stabil für $\beta =0 $            | 
|            |                         | instabil $\beta > 0$               |
